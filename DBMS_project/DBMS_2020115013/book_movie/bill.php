...
$foodamount=$_SESSION["food-amount"];
?>
<!DOCTYPE html>
<html lang="en">
<head>
...
<body>
    ...
    <main class="bill">
        <h3 class="bill-title">Your Booking Summary</h3>
        <div class="bill-flex">
            <div class="bill-part">
                <?php
                $x=$_SESSION["Movie_ID"];
$q = <<<EOF
    ...
    while($row=pg_fetch_row($result))
    {
        ...
        echo "<img src='../img/movie/$ans.jpeg' class='bill-img'>";
?>
            </div>
            <div class="bill-part">
                <div class="bill-details">
                    <div class="bill-detail-line">Movie Name :
                    <?php echo $moviename;?></div>
                    <div class="bill-detail-line">Show Date :
                        <?php echo $_SESSION["date-selected"];?>
                    </div>
                    <div class="bill-detail-line">Show Time :
                        <?php echo $_SESSION["time-selected"];?>
                    </div>
                    <div class="bill-detail-line">Screen Number :
                        <?php 
                        $q = <<<EOF
                        Select * from "Screen";
                        EOF;
                        ...
                        while($row=pg_fetch_row($result))
                        {
                            if($row[2]==$_SESSION["time-selected"] && $row[3]==$_SESSION["date-selected"])
                            {
                                echo $row[0];
                                break;
                            }
                    ...
                    <?php
                    $temp=substr($_SESSION["seats-selected"],0,strlen($_SESSION["seats-selected"])-2);
                    $price=$_SESSION["no-seats"]*200+$_SESSION["food-amount"];
                    ?>
                    <div class="bill-detail-line">Seats :
                        <?php echo $temp;?>
                    </div>
                    <div class="bill-detail-line">Food & Beverages :
                        <?php echo "&#8377;".$_SESSION["food-amount"];?>
                    </div>
                    <div class="bill-detail-line totalamt">Total Amount :
                        <?php echo "&#8377;".$price;?>
                    </div>
                    <form method="post">
                    <div class="bill-detail-line">
                        <input type="submit" value="Pay" class="pay-button" name="submitted">
                    </div>
                    </form>
                </div>
            </div>
    ...
    if(isset($_POST["submitted"]))
    {
        ...
        $aa=$_SESSION["show-id"];
        $bb=$_SESSION["Cust_id"];
        $cc=$_SESSION["date-selected"];
        $dd=$_SESSION["time-selected"];
        $ins = <<<EOF
        INSERT INTO "Booking" values('$counter','$aa','$bb','$price','$book_date','$temp','$cc','$dd','$moviename');
        EOF;
